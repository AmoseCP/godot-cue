...
func _ready() -> void:
	...
	_mouth.shape_textures = _make_textures()
	add_child(_mouth)
...
func _report() -> void:
	print("[口型示例] mouth 轨共 %d 条口型" % _mouth.entry_count())


func _build_face() -> void:
	_sprite = Sprite2D.new()
	_sprite.position = Vector2(320, 250)
	add_child(_sprite)
...
func _draw() -> void:
	...
	# 一张极简的脸,只是为了让嘴有个上下文
	draw_circle(Vector2(320, 220), 150.0, Color(0.96, 0.83, 0.70))
	draw_circle(Vector2(268, 175), 16.0, Color(1, 1, 1))
	draw_circle(Vector2(372, 175), 16.0, Color(1, 1, 1))
	draw_circle(Vector2(268, 177), 7.0, Color(0.15, 0.12, 0.10))
	draw_circle(Vector2(372, 177), 7.0, Color(0.15, 0.12, 0.10))

	var font := ThemeDB.fallback_font
	draw_string(font, Vector2(16, 28),
		"CueMouthShape 示例 —— Rhubarb JSON → mouth 轨 → Sprite2D",
		HORIZONTAL_ALIGNMENT_LEFT, -1, 15, Color(0.75, 0.8, 0.9))
	var t: float = Cue.time()
	draw_string(font, Vector2(16, 52),
		"t = %.2fs   frame = %d   当前嘴型 = %s" % [t, Cue.frame(), _mouth.current_shape()],
		HORIZONTAL_ALIGNMENT_LEFT, -1, 15, Color(0.6, 0.65, 0.75))
	for i in _log.size():
		draw_string(font, Vector2(16, 84 + float(i) * 18.0), _log[i],
